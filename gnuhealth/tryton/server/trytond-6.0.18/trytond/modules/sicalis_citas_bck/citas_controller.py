...
<body>
    <div style="width: 60%; max-width: 800px; min-width: 300px; margin: 0 auto;">
        <div style="display: flex; margin-bottom: 1rem;">
            <img src="https://cdn-icons-png.flaticon.com/512/212/212804.png" style="width: 50px; height: 50px; position: absolute; left: 20px; top: 10px; padding: 0px;">
            <div style="width: 100%; text-align: right;">
                <label for="codigoCita" style="display: inline-block; margin-bottom: .5rem; font-size: 20px; color: #222222;">Código de Cita: </label> <p style="color: blue; display: inline-block; margin-bottom: .5rem; font-size: 20px;">{token}</p>
            </div>
        </div>
...
def loadHistory(request):
    with Transaction().start(database_name, 1, context=CONTEXT):
        ...
        print("Abajo de este print ya no esta el problema")
        ...
        for cita in appointmentsSearch:
        ...
        # for appointment in appointmentsSearch:
            ...
            # healthprof_name = appointment.healthprof.name

            # response_dict['appointments'].append({
    # })

        print('Si llegaste hasta aqui es porque lo arreglaste, felicidades')
        json_response = json.dumps(response_dict)
        return Response(json_response, mimetype='application/json', status=200)
    
    
#Nombre, apellido, especialidad, fecha, hora, status, tipo, prof de salud,
                # 'Apellido': patient_lastname,
                # 'Especialidad': speciality_name,
                # 'Date': appointment_date,
                # 'Estatus': state,
